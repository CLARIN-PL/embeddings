    ...
    def build(
        ...
        if word_embedding_flag:
            ...
            if external_embedding_model is not None:
                ...
                external_emb, ext_emb_size = load_word_embeddings(
                ...
                embedding_layer = Embedding(
                    word_vocab_size, word_embedding_dims, input_length=sentence_length
                )
        ...
        if char_embedding_flag:
            word_chars_input = Input(
                shape=(sentence_length, word_length), name="word_chars_input"
            )
            all_inputs.append(word_chars_input)
            char_embedding_layer = Embedding(
                char_vocab_size, char_embedding_dims, input_length=word_length
            )
            char_embeddings = TimeDistributed(char_embedding_layer)(word_chars_input)
            if bilstm_layer:
                char_embeddings = TimeDistributed(Bidirectional(LSTM(word_lstm_dims)))(
                    char_embeddings
                )
            else:
        ...
        if crf_layer:
            if bilstm_layer:
                lstm_layers = Bidirectional(
                    LSTM(tagger_lstm_dims, return_sequences=True)
                )(features)
            else:
