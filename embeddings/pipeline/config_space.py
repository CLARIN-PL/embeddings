...
class LightningConfigSpace(
    ...
    def __post_init__(self) -> None:
        self.parse_fields_to_kwargs("task_train_kwargs", kwargs_group.LightningTaskTrainKwargs.__dataclass_fields__)
        self.parse_fields_to_kwargs("datamodule_kwargs", kwargs_group.DatamoduleKwargs.__dataclass_fields__)
        self.parse_fields_to_kwargs("task_model_kwargs", kwargs_group.TaskModelKwargs.__dataclass_fields__)
    ...
    def update_params(self, additional_params: dict) -> None:
        for param_name, param_value in additional_params.items():
            if not hasattr(self, param_name):
                _logger.warning("Parameter: {} do not belong to any group with parameters it will be assigned to other kwargs".format(param_name))
    ...
    def update_specific_params_group(self, additional_params: dict, params_group_name: str) -> None:
        if not hasattr(self, params_group_name):
            _logger.error("Unssupported group of params was selected: {}!".format(params_group_name))
